...
export function BetaSignupForm() {
  ...
  if (isSubmitted) {
    return (
      <div
        className="rounded-3xl border border-emerald-100 bg-emerald-50/40 p-5 sm:p-6"
        role="status"
        aria-live="polite"
      >
    ...
    >
      <div>
        ...
        <input
          id="beta-email"
          name="email"
          className="mt-2 h-12 w-full rounded-2xl border border-slate-200 bg-white px-4 text-base text-zinc-950 outline-none transition-colors duration-150 placeholder:text-zinc-400 focus:border-slate-400 focus:ring-2 focus:ring-slate-200"
          type="email"
          value={email}
      ...
      <fieldset className="mt-5">
        ...
        <div className="mt-3 space-y-2.5">
          {intentOptions.map((option) => (
            <label
              className="flex cursor-pointer items-start gap-3 rounded-2xl border border-slate-100 bg-white px-3 py-2.5 text-sm leading-6 text-zinc-700 transition-colors duration-150 has-[:checked]:border-slate-300 has-[:checked]:bg-slate-50"
              key={option}
            >
              <input
      ...
      <div className="mt-5">
        ...
        <textarea
          id="beta-note"
          name="note"
          className="mt-2 min-h-24 w-full resize-y rounded-2xl border border-slate-200 bg-white px-4 py-3 text-base leading-7 text-zinc-950 outline-none transition-colors duration-150 placeholder:text-zinc-400 focus:border-slate-400 focus:ring-2 focus:ring-slate-200"
          value={note}
